...
Version: 1.0.8 - Drag-and-Drop to Browser Uploads (GitHub, etc.)

A cross-platform file manager built with PyQt5, supporting Windows, macOS, and Linux.


WHAT'S NEW IN 1.0.8 (August 2025):
- Added open in new tab to selected folder right click menu
- Fixed bug that caused view menu to be doubled
...
class SimpleFileManager(QMainWindow):
    ...
    def setup_enhanced_menus(self):
        ...
        self.icon_view_pref_action = QAction("Icon View: Use simple icons", self, checkable=True)
        try:
            self.icon_view_pref_action.setChecked(bool(getattr(self, 'icon_view_use_icons_only', True)))
        except Exception:
            self.icon_view_pref_action.setChecked(True)
        self.icon_view_pref_action.triggered.connect(self.toggle_icon_view_icons_only)
        view_menu.addSeparator()
        view_menu.addAction(self.icon_view_pref_action)
        ...
        if current_tab:
            if mode == ViewModeManager.THUMBNAIL_VIEW:
                ...
                try:
                    ...
                    pass
                current_tab.view_stack.setCurrentWidget(current_tab.thumbnail_view_widget)
                try:
                    current_tab.refresh_current_view()
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                # Icon view uses the same widget as thumbnail view but instructs IconWidget to render icons only
                current_tab.view_stack.setCurrentWidget(current_tab.thumbnail_view_widget)
                try:
                    current_tab.refresh_current_view()
                except Exception:
    ...
    def show_about_dialog(self):
        ...
        about_lines = [
            "Gary's File Manager",
            "Version 1.1.0 - Icon View & Thumbnail refinements",
            "Release: August 2025",
            "",
            "Announcement: This release brings new usability features, UI improvements, and bug fixes for a smoother file management experience.",
            "",
            "🌟 WHAT'S NEW IN 1.1.0:",
            "• New Icon View: lightweight icons-only mode for faster browsing",
            "• Icon View reuses the thumbnail grid layout but avoids thumbnail generation",
            "• Added user preference: 'Icon View: Use simple icons' (persisted)",
            "• Immediate refresh when switching view modes",
            "• Thumbnail precache and thumbnail loading disabled while Icon View is active",
            "• Improved folder preview compositing remains available in Thumbnail View",
