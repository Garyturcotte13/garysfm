...
class SmbBrowserTab(QWidget):
    ...
    def on_item_double_clicked(self, item):
        ...
        def on_finished(success):
            ...
            thread.wait()
        thread = QThread()
        worker = Worker(self.smb, new_path)
        worker.moveToThread(thread)
        worker.finished.connect(on_finished)
        thread.started.connect(worker.run)
        thread.start()
...
Version: 1.1.3 - Recursive precache, thread-safe thumbnailing, always-show progress dialog, and more
...
class DirectorySelectionDialog(QDialog):
    ...
    def __init__(self, title="Select Directory", initial_dir=None, parent=None):
        print("[DEBUG] DirectorySelectionDialog.__init__ called")
        super().__init__(parent)
    ...
    def setup_ui(self):
        """Setup the dialog UI"""
        print("[DEBUG] DirectorySelectionDialog.setup_ui called")
        layout = QVBoxLayout(self)
                ...
                for ftype, fpath in preview_files:
                    ...
                    try:
                        ...
                        if ftype == 'image':
                            ...
                            # Use the file type icon for the ISO as a thumbnail
                            icon = icon_provider.icon(QFileInfo(fpath))
                            if icon.isNull():
                                icon = icon_provider.icon(QFileIconProvider.File)
                            thumbnail = icon.pixmap(preview_size, preview_size)
...
class FileManagerTab(QWidget):
    ...
    def setup_tab_ui(self):
        ...
        # Initialize with current folder or remote location
...
class SimpleFileManager(QMainWindow):
        ...
        try:
            ...
            if result == QDialog.Accepted:
                ...
                if selected_items:
                    ...
                    print("[DEBUG] Instantiating DirectorySelectionDialog in browse_archive_contents")
    ...
    def extract_archive_dialog(self, archive_path):
        ...
        try:
            ...
            print("[DEBUG] Instantiating DirectorySelectionDialog in extract_archive_dialog")
            dir_dialog = DirectorySelectionDialog(
                "Select Extract Location",
                default_extract_dir,
                self
    ...
    def empty_space_right_clicked(self, global_pos):
        ...
        print(f"[DEBUG] empty_space_right_clicked called with global_pos={global_pos}")
        context_menu = QMenu(self)
    ...
    def show_about_dialog(self):
        ...
        about_lines = [
            ...
            "Release: August 2025",
            "",
            "Announcement: This release adds recursive thumbnail precache, always-shown progress dialog, and thread-safe thumbnailing.",
            "",
            "🌟 WHAT'S NEW IN 1.1.3:",
            "• Tools → Recursive Precache Thumbnails... for multi-size, fast thumbnail generation",
            "• Always shows a progress dialog during thumbnail caching (no more preferences dialog)",
            "• Thread-safe thumbnail generation (no more Qt thread warnings)",
