...
Version: 1.1.1 - Multi-tab view sync, header styling, and fixes

A cross-platform file manager built with PyQt5, supporting Windows, macOS, and Linux.


WHAT'S NEW IN 1.1.1 (August 2025):
- All tabs now switch view modes together (thumbnail, icon, list, detail)
- Detail mode: column headers and line numbers (vertical header) now show in green
- Icon and thumbnail modes refresh all tabs and include a 1s delayed refresh to ensure thumbnails update
- Fixed startup AttributeError by hardening toolbar/action access and initialization order
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if is_empty_space:
                ...
                clicked_widget = clicked_widget if 'clicked_widget' in locals() else self.childAt(event.pos())
                try:
...
class SimpleFileManager(QMainWindow):
    ...
    def show_about_dialog(self):
        ...
        about_lines = [
            ...
            "Release: August 2025",
            "",
            "Announcement: This release improves multi-tab view syncing, stabilizes view-mode persistence, and includes UI styling and crash hardening.",
            "",
            "🌟 WHAT'S NEW IN 1.1.1:",
            "• All view modes (Thumbnail, Icon, List, Detail) now apply to every open tab",
            "• Thumbnail and Icon modes run thumbnail logic and refresh across all tabs (includes a 1s delayed refresh)",
            "• Detail mode header text and line numbers (vertical header) styled green for better readability",
            "• Added icons support in List and Detail views",
            "• Persist view mode selection across sessions",
            "• Fixed startup AttributeError crashes by hardening toolbar/action and tab initialization",
            "• Improved refresh behavior so non-active tabs stay in sync when switching modes",
            "• Misc UI polish and bug fixes (performance and stability)",
            "",
            "🚀 FEATURES (including previous versions):",
            "• Video thumbnailing for major formats (mp4, mkv, avi, mov, etc.)",
            "• ffmpeg-based thumbnail extraction (cross-platform)",
