...
Version: 0.6.1 - Enhanced Exit Handling & UI Improvements
...
and optimized asynchronous file handling for seamless user experience.
...
class ThumbnailCache:
    ...
    def get(self, file_path, size):
        """Get cached thumbnail with thread safety"""
    ...
    def put(self, file_path, size, thumbnail_data):
        """Store thumbnail in cache with thread safety"""
        cache_key = self.get_cache_key(file_path, size)
        
        # Store in memory cache (thread-safe)
        self._add_to_memory_cache(cache_key, thumbnail_data)
        
        # Store on disk (I/O outside critical section)
        try:
            cache_file = os.path.join(self.cache_dir, f"{cache_key}.thumb")
    ...
    def cleanup(self):
        ...
        try:
            # Clear memory cache
            print(f"Clearing thumbnail memory cache ({len(self.memory_cache)} items)...")
            self.memory_cache.clear()
            ...
            print("Thumbnail cache cleanup complete")
        except Exception as e:
...
class VirtualFileLoader:
    ...
    def cleanup(self):
        ...
        try:
            print("Cleaning up VirtualFileLoader...")
            ...
            if hasattr(self, 'executor') and self.executor:
                try:
                    ...
                    print("VirtualFileLoader ThreadPoolExecutor shutdown complete")
                except Exception as e:
            ...
            print("VirtualFileLoader cleanup complete")
...
class MemoryManager:
            ...
            while self.running:  # Check running flag instead of infinite loop
                try:
                    ...
                    if time.time() - self.last_cleanup > self.check_interval:
                        print(f"Routine cleanup triggered. Current memory: {memory_mb:.1f} MB")
                    ...
                    if not hasattr(self, '_last_memory_check'):
                        ...
                        if memory_growth > 50 * 1024 * 1024:  # 50MB growth
                            ...
                            print(f"Memory growth detected: +{memory_growth/1024/1024:.1f} MB")
                            if self._memory_growth_counter >= 3:  # 3 consecutive growths
                                print("Potential memory leak detected, forcing cleanup...")
                                self.force_cleanup()
                                self._memory_growth_counter = 0
                        else:
                            self._memory_growth_counter = 0
    ...
    def routine_cleanup(self):
        ...
        try:
            ...
            # Force garbage collection with cycle detection
            import gc
            ...
            if gc.garbage:
                ...
                # Clear the garbage list to prevent memory buildup
                gc.garbage.clear()
    ...
    def force_cleanup(self):
        ...
        try:
            ...
            try:
                ...
                process = psutil.Process()
                memory_before = process.memory_info().rss / 1024 / 1024  # MB
                print(f"Memory usage before aggressive cleanup: {memory_before:.1f} MB")
            except ImportError:
                memory_before = 0
...
class SearchFilterWidget(QWidget):
    ...
    def setup_ui(self):
        """Set up the search filter interface"""
        ...
        self.filters_group = QGroupBox("Advanced Filters")
        self.filters_group.setCheckable(True)
        self.filters_group.setChecked(False)
        filters_layout = QGridLayout(self.filters_group)
        
        # File type filter
        filters_layout.addWidget(QLabel("File Type:"), 0, 0)
        self.type_combo = QComboBox()
        self.type_combo.addItems(['All', 'Image', 'Video', 'Audio', 'Document', 'Code', 'Archive', 'Executable'])
        filters_layout.addWidget(self.type_combo, 0, 1)
        
        # Size filter
        filters_layout.addWidget(QLabel("Size:"), 1, 0)
    ...
    def __init__(self):
        ...
        def _cleanup_thumbnails():
            ...
            try:
                ...
                print(f"Error in thumbnail cleanup: {e}")
