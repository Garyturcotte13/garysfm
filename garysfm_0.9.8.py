...
class FileManagerTab(QWidget):
    ...
    def browse_ftp(self, url):
        ...
        try:
            ...
            # Add to navigation history if new
    ...
    def go_forward(self):
        ...
        if self.can_go_forward():
            self.history_index += 1
            path = self.navigation_history[self.history_index]
            # If it's a remote URL, trigger the correct browser
            if isinstance(path, str) and path.startswith('ftp://'):
                self.browse_ftp(path)
            elif isinstance(path, str) and path.startswith('sftp://'):
                self.browse_sftp(path)
            else:
                self.navigate_to(path, add_to_history=False)
            if hasattr(self, 'address_bar'):
                self.address_bar.setText(path)
...
class SimpleFileManager(QMainWindow):
    ...
    def show_clipboard_history(self):
        # Update clipboard history button text after dialog closes to show current count.
        if hasattr(self, 'clipboard_history_action'):
            count = len(self.clipboard_manager.get_history())
            self.clipboard_history_action.setText(f"[] Clipboard ({count})")
        # ...existing code...
