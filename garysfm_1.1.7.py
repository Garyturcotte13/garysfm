...
# NOTE: Global replacement of builtins.print hides exceptions and is fragile.
# Keep the filtered helper available for targeted use, but do NOT override
# builtins.print globally. Use the logging module instead.
...
def _qt_message_handler(mode, context, message):
    try:
        ...
        pass
    # Fallback: log Qt messages through the logging system instead of printing
    try:
        logging.getLogger('qt').debug(message)
    except Exception:
        # As a last resort, use the original print
    ...
    def cache_one_file(file_path):
        ...
        try:
            if is_thumb_file(file_path):
                print(f"[THUMBNAIL-PRECACHE] Skipping .thumb cache file: {file_path}")
                return
        except Exception:
            pass
    ...
    try:
        ...
        if getattr(e, 'errno', None) == errno.EXDEV:
            try:
                ...
                import traceback
                logging.getLogger('thumbnail').exception('Exception in waveform extraction for %s: %s\n%s', wav_path, e, traceback.format_exc())
                logging.getLogger('thumbnail').debug('Returning None at exception in waveform extraction for %s', wav_path)
    ...
    def create_icon_or_thumbnail(self, full_path, is_dir):
        ...
        try:
            if is_thumb_file(full_path):
                print(f'[THUMBNAIL-DEBUG] Skipping thumbnail generation for .thumb file: {full_path}')
                framed_pixmap = QPixmap(size, size)
                framed_pixmap.fill(Qt.transparent)
                painter = QPainter(framed_pixmap)
                try:
...
class SimpleFileManager(QMainWindow):
    ...
    def show_about_dialog(self):
        ...
        about_lines = [
            "Gary's File Manager",
            "Version 1.1.7 - ISO thumbnail fixes, thumbnail cache improvements, and UI polish",
            "Release: August 2025",
            "",
            "🚀 FEATURES:",
