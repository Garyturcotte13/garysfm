...
Version: 0.6.0 - Optimized UI Responsiveness & Async Operations
...
class SimpleFileManager(QMainWindow):
    ...
    def stop_background_operations(self):
        """Stop all background operations and timers"""
        try:
            # Stop memory manager if it exists
            if hasattr(self, 'memory_manager') and self.memory_manager:
                if hasattr(self.memory_manager, 'stop'):
                    self.memory_manager.stop()
                elif hasattr(self.memory_manager, 'timer') and self.memory_manager.timer:
                    self.memory_manager.timer.stop()
            
            # Stop file monitor if it exists
            if hasattr(self, 'file_monitor') and self.file_monitor:
                if hasattr(self.file_monitor, 'stop'):
                    self.file_monitor.stop()
                elif hasattr(self.file_monitor, 'timer') and self.file_monitor.timer:
                    self.file_monitor.timer.stop()
