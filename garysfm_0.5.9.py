...
Version: 0.5.9 - Enhanced Progress Dialogs & File Operations

A cross-platform file manager built with PyQt5, supporting Windows, macOS, and Linux.
This version includes enhanced asynchronous file operations with improved progress dialogs,
pause/cancel functionality, and Properties dialog for comprehensive file information.
...
class AsyncFileOperationWorker(QThread):
    ...
    def _async_copy_file(self, source_path, dest_path):
        ...
        if os.path.exists(dest_path):
            if not self.operation.overwrite_all and not self.operation.skip_all:
                self.confirmationNeeded.emit(
                    "File Exists", 
                    f"'{dest_path}' already exists. What would you like to do?",
                    dest_path
                )
                # This would need to be handled by the UI thread
                return
            elif self.operation.skip_all:
                return
...
class EnhancedProgressDialog(QDialog):
    ...
    def toggle_pause(self):
        """Toggle pause/resume operation"""
        if self.operation and self.operation_worker:
            if self.operation.paused:
                self.operation.resume()
                self.pause_button.setText("Pause")
                self.status_label.setText("Resuming operation...")
                self.status_label.setStyleSheet("color: green; font-weight: bold;")
            else:
                self.operation.pause()
                self.pause_button.setText("Resume")
                self.status_label.setText("Operation paused")
                self.status_label.setStyleSheet("color: orange; font-weight: bold;")
        else:
            # Provide immediate feedback even if operation isn't available
            self.status_label.setText("Cannot pause - no active operation")
    ...
    def closeEvent(self, event):
        """Handle dialog close event to cancel operation"""
        if self.operation and not self.operation.cancelled:
            reply = QMessageBox.question(
                self, 
                "Cancel Operation",
                "An operation is in progress. Do you want to cancel it?",
...
class SimpleFileManager(QMainWindow):
    ...
    def paste_action_triggered(self):
        ...
        self.paste_multiple_items(paths, destination, operation)
        ...
        if current_tab:
            ...
            if icon_container:
                ...
                for i in range(layout.count()):
                    ...
                    if item and item.widget():
                        widget = item.widget()
                        if hasattr(widget, 'full_path') and hasattr(icon_container, 'add_to_selection'):
                            icon_container.add_to_selection(widget)
    ...
    def handle_dropped_files(self, file_paths, target_dir):
        ...
        try:
            ...
            if safe_files:
                ...
                if reply == QMessageBox.Yes:
                    # Use the enhanced async file operation system to move files
                    operation = AsyncFileOperation(safe_files, target_dir, "move")
                    dialog = EnhancedProgressDialog("Move Operation", len(safe_files), self)
                    worker = AsyncFileOperationWorker(operation)
                    
                    worker.progress.connect(dialog.update_progress)
                    worker.fileProgress.connect(dialog.update_file_progress)
                    worker.byteProgress.connect(dialog.update_byte_progress)
                    worker.speedUpdate.connect(dialog.update_speed)
                    worker.etaUpdate.connect(dialog.update_eta)
...
def main():
    ...
    app.setApplicationVersion("0.5.9")
