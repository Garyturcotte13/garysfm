...
# Configure basic logging for debug statements introduced during development
...
def get_waveform_thumbnail(wav_path, width=128, height=48, color=QColor('deepskyblue'), thumbnail_cache=None):
    ...
    try:
        ...
        # Try using wave module first (WAV only), then soundfile (for FLAC and others), then pydub (for MP3)
        try:
            if wav_path.lower().endswith('.wav'):
                # First try the stdlib wave backend
                try:
                    ...
                    # If the wave backend fails for some WAV files, try soundfile as a fallback
                    print(f"[WAVEFORM-ERROR] wave backend failed for {wav_path}: {e_wave_backend}")
                    try:
                        import soundfile as sf
                        print(f"[THUMBNAIL-DEBUG] Using soundfile fallback to read: {wav_path}")
                        data, samplerate = sf.read(wav_path)
                        if hasattr(data, 'ndim') and data.ndim > 1:
                            data = data[:, 0]
                        data = data.tolist() if hasattr(data, 'tolist') else list(data)
                        print(f"[THUMBNAIL-DEBUG] Finished soundfile fallback for: {wav_path}")
                    except Exception as e_sf_fallback:
                        print(f"[WAVEFORM-ERROR] soundfile fallback failed for {wav_path}: {e_sf_fallback}")
                        print(f"[THUMBNAIL-DEBUG] Returning None at wav+soundfile fallback failure")
                        return None
            else:
                print(f"[THUMBNAIL-DEBUG] Entering soundfile backend for: {wav_path}")
                # Use soundfile for FLAC and other supported formats
                try:
                    import soundfile as sf
                    ...
                    print(f"[WAVEFORM-ERROR] soundfile backend failed for {wav_path}: {e_sf}")
                    # Try pydub for mp3 only
                    if wav_path.lower().endswith('.mp3'):
                        try:
                            ...
                            import traceback
                            print(f"[WAVEFORM-ERROR] All backends failed for {wav_path}:\nsoundfile: {e_sf}\nPydub: {e_pydub}\n{traceback.format_exc()}")
                            print(f"[THUMBNAIL-DEBUG] Returning None at all backends failed (mp3)")
                            return None
                    else:
                        import traceback
                        print(f"[WAVEFORM-ERROR] All non-wave backends failed for {wav_path}:\nsoundfile: {e_sf}\n{traceback.format_exc()}")
                        print(f"[THUMBNAIL-DEBUG] Returning None at all non-wave backends failed")
                        return None
...
def get_exe_icon_qicon(exe_path, size=32):
    ...
    if sys.platform.startswith('win'):
        ...
        painter.end()
        return QIcon(pixmap)
def precache_text_pdf_thumbnails_in_directory(directory, thumbnail_cache, size=128, max_workers=4, on_complete=None):
    """
...
Version: 1.1.2 - 640/768 thumbnails, empty-space context fixes, and selection fixes

A cross-platform file manager built with PyQt5, supporting Windows, macOS, and Linux.


WHAT'S NEW IN 1.1.2 (August 2025):
- Added support for larger thumbnail sizes (640px and 768px) and corresponding UI actions
- Improved empty-space click handling: right-click context menu (including Paste) now appears consistently
- Fixed rubberband selection and tab-signal handling so selection works after switching tabs
- Minor UI polish and reliability fixes
...
class SimpleFileManager(QMainWindow):
        ...
        if current_tab:
            if mode == ViewModeManager.THUMBNAIL_VIEW:
                ...
                try:
                    ...
                    pass
                # Add a 1-second delayed refresh for all tabs
                from PyQt5.QtCore import QTimer
                QTimer.singleShot(1000, lambda: [getattr(tab, 'refresh_current_view', lambda: None)() for tab in getattr(self.tab_manager, 'tabs', [])])
    ...
    def show_about_dialog(self):
        ...
        about_lines = [
            ...
            "Release: August 2025",
            "",
            "Announcement: This release adds larger thumbnail sizes, improves empty-space right-click handling, and fixes selection behavior across tabs.",
            "",
            "🌟 WHAT'S NEW IN 1.1.2:",
            "• Added UI options and support for 640px and 768px thumbnails",
            "• Right-click in empty folder space now consistently shows the folder context menu (Paste when applicable)",
            "• Fixed rubberband selection and ensured selection works after switching tabs",
