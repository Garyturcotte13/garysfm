def clear_all_thumbnail_cache(main_window):
    """Utility to clear all thumbnail cache files from the main window's cache."""
    if hasattr(main_window, 'thumbnail_cache') and main_window.thumbnail_cache:
        main_window.thumbnail_cache.clear_cache_files()
    else:
        pass
...
def precache_video_thumbnails_in_directory(directory, thumbnail_cache, size=128, max_workers=4):
    ...
    for f in all_files:
        ...
        if is_video:
            cache_key = thumbnail_cache.get_cache_key(f, size)
            png_path = os.path.join(thumbnail_cache.cache_dir, f"{cache_key}.png")
            valid_png = False
            if os.path.exists(png_path):
                try:
                    ...
                    pass
            if not valid_png:
                # Generate PNG thumbnail using ffmpeg
                ffmpeg_path = shutil.which('ffmpeg')
                if ffmpeg_path:
                    # Use ffmpeg to extract a frame at 10%% duration
                    import subprocess
                    import json
                    try:
                        # Get video duration
                        probe_cmd = [ffmpeg_path, '-v', 'error', '-show_entries', 'format=duration', '-of', 'json', '-i', f]
                        ...
                        # Extract frame
                        thumb_cmd = [ffmpeg_path, '-ss', str(seek_time), '-i', f, '-vframes', '1', '-vf', f'scale={size}:{size}:force_original_aspect_ratio=decrease', '-f', 'image2', png_path, '-y']
    ...
    # Only create ProcessPoolExecutor and submit jobs if in main process
    import sys
    if hasattr(sys, 'frozen') or __name__ == '__main__':
        import concurrent.futures
        from PyQt5.QtCore import QTimer
    # ...removed THUMBNAIL-DEBUG print...
    ...
    # ...removed print...
        for video_path in video_files:
            ...
            # ...removed print...
            try:
                future = executor.submit(extract_thumbnail_worker, (video_path, size))
                # ...removed print...
            except Exception as e:
                continue
            # Use QTimer.singleShot to poll for result and update UI in main thread
            def check_future(fut=future):
                if fut.done():
                    pass
                else:
                    QTimer.singleShot(100, lambda: check_future(fut))
            QTimer.singleShot(0, lambda fut=future: check_future(fut))
...
def extract_thumbnail_worker(args):
    video_path, size = args
    try:
        import ffmpeg
        from PIL import Image
        import tempfile
        ...
        (
            ...
            .output(tmp_jpg_path, vframes=1, format='image2', vcodec='mjpeg')
            .overwrite_output()
            .run(quiet=True)
        )
        img = Image.open(tmp_jpg_path)
        img = img.convert('RGBA').resize((size, size), Image.LANCZOS)
        # Save as PNG to a new temp file
        ...
        return (video_path, size, data)
    except Exception as e:
        return (video_path, size, None)
...
Version: 0.8.0 - Video Thumbnails, ffmpeg Support, "Open with..." & Cross-platform Improvements
...
def format_filename_with_underscore_wrap(filename, max_length_before_wrap=20):
    ...
    Replaces underscores with a zero-width space followed by underscore
                handle_result(fut, thumbnail_cache)
...
class ThumbnailCache:
    def clear_cache_files(self):
        """Delete all thumbnail cache files and clear metadata."""
        try:
            for filename in os.listdir(self.cache_dir):
                if filename.endswith('.thumb') or filename == 'cache_metadata.json':
            ...
            with self._lock:
                ...
                self.metadata.clear()
            self._save_metadata()
            pass  # ...removed THUMBNAIL-DEBUG print...
        except Exception as e:
            print(f'[THUMBNAIL-ERROR] Failed to clear cache files: {e}')
    # Add: allow listeners to be notified when a thumbnail is added
    def __init__(self, cache_dir=None):
        # ...existing code...
        self._thumbnail_update_callbacks = []  # List of callbacks (file_path, size)

    def add_thumbnail_update_callback(self, callback):
        if not hasattr(self, '_thumbnail_update_callbacks'):
            self._thumbnail_update_callbacks = []
        self._thumbnail_update_callbacks.append(callback)

    def notify_thumbnail_update(self, file_path, size):
        if not hasattr(self, '_thumbnail_update_callbacks'):
            self._thumbnail_update_callbacks = []
        for cb in self._thumbnail_update_callbacks:
            try:
                cb(file_path, size)
            except Exception:
                pass
    ...
    def get(self, file_path, size):
        """Get cached thumbnail as QPixmap from PNG file"""
        import os
        from PyQt5.QtGui import QPixmap
        cache_key = self.get_cache_key(file_path, size)
        png_path = os.path.join(self.cache_dir, f"{cache_key}.png")
    # ...removed THUMBNAIL-DEBUG print...
        if os.path.exists(png_path):
            ...
            if not pixmap.isNull():
                ...
                print(f"[THUMBNAIL-ERROR] PNG file exists but QPixmap isNull: {png_path}")
        else:
            pass
        return None
    ...
    def put(self, file_path, size, thumbnail_data):
        # No-op: PNG files are now written directly in handle_result
        pass
...
class IconWidget(QWidget):
    ...
    # ...removed THUMBNAIL-DEBUG print...
        ...
        # Check if we should display video thumbnails
        show_video_thumbnails = True
        parent = self.parent()
        while parent is not None:
            if hasattr(parent, 'show_video_thumbnails'):
                show_video_thumbnails = parent.show_video_thumbnails
                break
            parent = parent.parent() if hasattr(parent, 'parent') else None
        video_exts = {'.mp4', '.avi', '.mkv', '.mov', '.wmv', '.flv', '.webm', '.m4v'}
        file_ext = os.path.splitext(full_path)[1].lower()
        if self.thumbnail_cache and not is_dir:
            # If this is a video and thumbnails are disabled, show default icon
            if file_ext in video_exts and not show_video_thumbnails:
                return self.draw_default_file_icon_pixmap(size)
            # ...removed THUMBNAIL-DEBUG print...
            cached_thumbnail = self.thumbnail_cache.get(full_path, size)
            if cached_thumbnail:
                # ...removed THUMBNAIL-DEBUG print...
                return cached_thumbnail
            # If not in cache, show a placeholder and schedule a refresh
            QTimer.singleShot(500, self.refresh_icon)
            return self.draw_default_file_icon_pixmap(size)
        # ...existing code for directories and images...
    ...
    def refresh_icon(self):
        # Called to refresh the icon if a new thumbnail is available
        pixmap = self.create_icon_or_thumbnail(self.full_path, self.is_dir)
        self.icon_label.setPixmap(pixmap)
        self.update()

    def draw_default_file_icon_pixmap(self, size):
        # Helper to return a default file icon pixmap of the right size
        framed_pixmap = QPixmap(size, size)
        framed_pixmap.fill(Qt.transparent)
        painter = QPainter(framed_pixmap)
        painter.setRenderHint(QPainter.Antialiasing)
        self.draw_default_file_icon(painter, self.full_path, size)
        painter.end()
        return framed_pixmap
...
class FileManagerTab(QWidget):
    ...
    def __init__(self, initial_path, tab_manager):
        ...
        # Register thumbnail update callback if thumbnail_cache exists
        if self.tab_manager and hasattr(self.tab_manager, 'main_window') and hasattr(self.tab_manager.main_window, 'thumbnail_cache'):
            self.tab_manager.main_window.thumbnail_cache.add_thumbnail_update_callback(self._on_thumbnail_update)

    def _on_thumbnail_update(self, file_path, size):
        # Called when a new thumbnail is available; refresh the icon if present
        import os
        ...
        for i in range(layout.count()):
            ...
            if item and item.widget():
                widget = item.widget()
                if hasattr(widget, 'full_path') and hasattr(widget, 'thumbnail_size'):
                    widget_path = os.path.normcase(os.path.abspath(widget.full_path))
                    if widget_path == normalized_file_path and widget.thumbnail_size == size:
                        widget.refresh_icon()
    def add_tab(self, initial_path):
        tab = FileManagerTab(initial_path, self)
        self.tabs.addTab(tab, os.path.basename(initial_path) or initial_path)
        # Register thumbnail update callback for the new tab
        if hasattr(self, 'main_window') and hasattr(self.main_window, 'thumbnail_cache'):
            self.main_window.thumbnail_cache.add_thumbnail_update_callback(tab._on_thumbnail_update)
        return tab
    ...
    def navigate_to(self, path, add_to_history=True):
        ...
        if os.path.exists(path) and os.path.isdir(path):
            ...
            # --- THUMBNAIL DEBUG: Ensure video thumbnail pre-caching is triggered on navigation ---
            try:
                # Try to get thumbnail_cache from main_window if not present on self
                thumbnail_cache = getattr(self, 'thumbnail_cache', None)
                if not thumbnail_cache and hasattr(self, 'tab_manager') and hasattr(self.tab_manager, 'main_window'):
                    thumbnail_cache = getattr(self.tab_manager.main_window, 'thumbnail_cache', None)
                if thumbnail_cache:
                    # ...removed THUMBNAIL-DEBUG print...
                    precache_video_thumbnails_in_directory(path, thumbnail_cache, size=128)
                else:
                    pass
            except Exception as e:
                pass
...
class SimpleFileManager(QMainWindow):
    ...
    # ...existing code for toolbar setup...
    ...
    def create_archive_dialog(self, source_paths):
        ...
        try:
            # Ask for output location and name using a non-native PyQt dialog
            ...
            from PyQt5.QtWidgets import QFileDialog
            file_dialog = QFileDialog(self, "Create Archive", os.path.join(default_dir, suggested_name))
    ...
    def set_thumbnail_size(self, size):
        ...
        current_path = getattr(self, 'current_folder', os.path.expanduser("~"))
        self.save_last_dir(current_path)
        # Trigger thumbnail generation for the new size in all open tabs
        from PyQt5.QtWidgets import QApplication
        main_window = self
        if hasattr(self, 'tab_manager'):
            for tab in self.tab_manager.tabs:
                # Only trigger for icon view tabs with a folder
                if hasattr(tab, 'current_folder') and hasattr(main_window, 'thumbnail_cache') and main_window.thumbnail_cache:
                    try:
                        precache_video_thumbnails_in_directory(tab.current_folder, main_window.thumbnail_cache, size=size)
                    except Exception as e:
                        print(f"[THUMBNAIL-ERROR] Exception triggering thumbnail generation for {tab.current_folder}: {e}")
