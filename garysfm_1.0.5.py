...
def get_waveform_thumbnail(wav_path, width=128, height=48, color=QColor('deepskyblue'), thumbnail_cache=None):
    print(f"[THUMBNAIL-DEBUG] get_waveform_thumbnail called for: {wav_path}")
    import os
    print(f"[THUMBNAIL-DEBUG] File exists: {os.path.exists(wav_path)} | Path: {wav_path}")
    print(f"[THUMBNAIL-DEBUG] File extension: {os.path.splitext(wav_path)[1].lower()}")
    """
        ...
        try:
            ...
            if ext in text_exts:
                ...
                try:
                    ...
                    pixmap = get_waveform_thumbnail(file_path, width=size, height=size, thumbnail_cache=self.thumbnail_cache)
...
Version: 1.0.5 - Folder Preview Compositing, ISO/Archive Thumbnails, and More

A cross-platform file manager built with PyQt5, supporting Windows, macOS, and Linux.

WHAT'S NEW IN 1.0.5 (August 2025):
- Folder preview compositing for all file types: up to four thumbnails or file type icons composited onto folder icons
- ISO and compressed files now appear as thumbnails in folder previews, with robust fallback logic
- Improved thumbnail and icon handling for mixed-content folders
- Enhanced fallback to file type icons for empty preview slots
...
class IconWidget(QWidget):
    ...
    def create_icon_or_thumbnail(self, full_path, is_dir):
        ...
        # Always use composite folder preview for directories
        if is_dir:
...
class FileManagerTab(QWidget):
    ...
    def setup_icon_view(self):
        ...
        # Scroll area to contain the icon grid
        ...
        # Icon container (grid of IconWidget instances)
...
class SimpleFileManager(QMainWindow):
    ...
    def handle_dropped_files(self, paths):
        ...
        # Example: print the dropped paths. Replace with your logic.
    ...
    def show_about_dialog(self):
        ...
        about_text += "Announcement: This release adds enhanced folder preview compositing, ISO/archive thumbnails, improved drag & drop (multi-item), and faster local cut/paste moves (uses rename when possible).\n\n"
        about_text += "🌟 WHAT'S NEW IN 1.0.5:\n"
        about_text += "• Folder preview compositing for all file types: up to four thumbnails or file type icons composited onto folder icons\n"
        about_text += "• ISO and compressed files now appear as thumbnails in folder previews, with robust fallback logic\n"
        about_text += "• Improved thumbnail and icon handling for mixed-content folders\n"
        about_text += "• Enhanced fallback to file type icons for empty preview slots\n"
        about_text += "• Improved drag & drop: multi-item dragging and drop-onto-folder moves\n"
        about_text += "• Paste behavior: pasting into a single selected folder now uses that folder as destination\n"
        about_text += "• Faster local moves: cut/paste uses atomic rename (os.replace) when source and destination are on the same filesystem\n"
        about_text += "• Bug fixes and performance improvements\n\n"

        about_text += "🚀 FEATURES (including previous versions):\n"
                        ...
                        try:
                            ...
                            try:
                                ...
                                if os.path.exists(trash_dir):
                                    fast_move(item_path, os.path.join(trash_dir, os.path.basename(item_path)))
                                    success_count += 1
                                else:
                                    raise apple_error
                    else:  # Linux
