...
Version: 1.1.6 - APK thumbnails, .xapk support, ISO EXE compositing, and other improvements

A cross-platform file manager built with PyQt5, supporting Windows, macOS, and Linux.



WHAT'S NEW IN 1.1.6 (August 2025):
- Improved: "My Computer" shows Windows special user folders (Documents, Downloads, Desktop, Music, Pictures, Videos) and lists drives first for easier access
- New (Windows only): Tools → Control Panel — quick access to the Control Panel from the Tools menu
                    ...
                    try:
                        ...
                        try:
                            if getattr(self, 'thumbnail_cache', None):
                                ...
                                print(f'[APK-ICON] Cached APK thumbnail for {full_path} ({len(png_bytes)} bytes)')
                        except Exception as e_cache:
                            print(f'[APK-ICON] Caching APK thumbnail failed: {e_cache}')
                        return base_pixmap
                    ...
                    try:
                        if file_ext == '.xcf':
                            try:
                                ...
                                print(f'[THUMBNAIL-DEBUG] XCF thumbnail error for {full_path}: {e}')
...
class SimpleFileManager(QMainWindow):
    ...
    def show_about_dialog(self):
        ...
        about_lines = [
            ...
            "Release: August 2025",
            "",
            "Announcement: This release adds APK/.xapk thumbnail extraction, caches generated APK thumbnails, and improves ISO thumbnail compositing.",
            "",
            "🌟 WHAT'S NEW IN 1.1.6:",
            "• Improved: 'My Computer' now shows Windows special user folders (Documents, Downloads, Desktop, Music, Pictures, Videos) and lists drives first for easier access",
            "• New (Windows only): Tools → Control Panel — quick access to the Control Panel from the Tools menu",
            "• APK thumbnail extraction and adaptive icon composition (extracts launcher icons from .apk and .xapk)",
            "• .xapk support (detect and use embedded APK for thumbnails)",
            "• Cache generated APK thumbnails for faster reloads",
            "• Improved ISO thumbnails – extract EXE icons and composite them over disc artwork; avoid blank/transparent icons",
