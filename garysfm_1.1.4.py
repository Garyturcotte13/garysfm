...
# Quiet noisy debug/precache console prints: wrap builtins.print to filter known debug prefixes
_original_print = _builtins.print
def _filtered_print(*args, **kwargs):
    try:
        msg = ' '.join(str(a) for a in args)
    except Exception:
        # Fallback to original print if joining fails
        return _original_print(*args, **kwargs)
    # List of noisy prefixes to suppress
    noisy_prefixes = (
        '[THUMBNAIL-',
        '[THUMBNAIL-PRECACHE]',
        '[THUMBNAIL-DEBUG]',
    ...
    # Also suppress messages that contain these noisy substrings (case-insensitive)
    noisy_substrings = (
        'tab widget',
        'qtabwidget',
    'tabwidget',
    ...
    'resource cleanup',
    'windows detected - using aggressive exit strategy',
    'windows cleanup - forcing exit',
    'debug',
    )
    for p in noisy_prefixes:
        if msg.startswith(p):
            # Drop noisy debug/precache message
            return
    # Drop messages containing noisy substrings (case-insensitive)
    lower_msg = msg.lower()
    for s in noisy_substrings:
        if s in lower_msg:
            return
    # Otherwise print normally
    return _original_print(*args, **kwargs)

# Replace builtins.print with the filtered variant
_builtins.print = _filtered_print

# Install a Qt message handler to suppress a known noisy warning
def _qt_message_handler(mode, context, message):
    try:
        # Filter tab-widget and lifecycle messages (case-insensitive)
        if isinstance(message, str):
            lower = message.lower()
            if ('tab widget' in lower or 'tabwidget' in lower or 'stopping' in lower
                or 'cleaning up' in lower or 'starting application shutdown' in lower
                or 'garbage collection freed' in lower or 'resource cleanup' in lower
                or 'aggressive exit' in lower or 'forcing exit' in lower):
                return  # drop these noisy Qt warnings/messages
    except Exception:
        pass
    # Fallback: let Qt handle other messages by printing them through the filtered print
    _original_print(message)

try:
    qInstallMessageHandler(_qt_message_handler)
except Exception:
    # If Qt doesn't expose the installer in this environment, ignore silently
    pass

# --- Minimal dependency waveform thumbnail for .wav files ---
from PyQt5.QtGui import QPixmap, QPainter, QColor
def get_waveform_thumbnail(wav_path, width=128, height=48, color=QColor('deepskyblue'), thumbnail_cache=None):
    logging.debug("[THUMBNAIL-DEBUG] get_waveform_thumbnail called for: %s", wav_path)
    import os
    logging.debug("[THUMBNAIL-DEBUG] File exists: %s | Path: %s", os.path.exists(wav_path), wav_path)
    logging.debug("[THUMBNAIL-DEBUG] File extension: %s", os.path.splitext(wav_path)[1].lower())
    """
    ...
    """
    try:
        # Use the application's thumbnail cache if available
        import os
        import hashlib
        # Use temp directory for cache, as in main app logic
        temp_dir = os.path.join(os.environ.get('TEMP', '/tmp'), 'garysfm_thumbnails')
        os.makedirs(temp_dir, exist_ok=True)
        # Cache key: md5 of file path, plus size, as in debug output
        ...
        if os.path.exists(thumb_path):
            print(f"[THUMBNAIL-CACHE] Found cache file: {thumb_path}")
            pixmap = QPixmap(thumb_path)
            if not pixmap.isNull():
                return pixmap
            else:
                print(f"[THUMBNAIL-CACHE] Cache file exists but is invalid: {thumb_path}")
        # Prefer soundfile for all audio types (WAV/FLAC/MP3 if supported).
        try:
            try:
                import soundfile as sf
                print(f"[THUMBNAIL-DEBUG] Using soundfile to read: {wav_path}")
                data, samplerate = sf.read(wav_path)
                if hasattr(data, 'ndim') and data.ndim > 1:
                    data = data[:, 0]
                data = data.tolist() if hasattr(data, 'tolist') else list(data)
                print(f"[THUMBNAIL-DEBUG] Finished soundfile block for: {wav_path}")
            except Exception as e_sf:
                # soundfile not available or failed; fall back to previous logic
                print(f"[THUMBNAIL-DEBUG] soundfile not available or failed for {wav_path}: {e_sf}")
                if wav_path.lower().endswith('.wav'):
                    # Try stdlib wave backend for WAV files
                    try:
                        with wave.open(wav_path, 'rb') as wf:
                            ...
                            # Only use first channel for mono display
                            if sampwidth == 1:
                                fmt = f"{n_frames * n_channels}B"
                                data = struct.unpack(fmt, frames)
                                data = [x - 128 for x in data]  # 8-bit WAV is unsigned
                            elif sampwidth == 2:
                                fmt = f"{n_frames * n_channels}h"
                                data = struct.unpack(fmt, frames)
                            else:
                                print(f"[WAVEFORM-ERROR] Unsupported sample width: {sampwidth} for {wav_path}")
                                print(f"[THUMBNAIL-DEBUG] Returning None at unsupported sample width")
                                return None
                            if n_channels > 1:
                                data = data[::n_channels]
                        print(f"[THUMBNAIL-DEBUG] Finished WAV block for: {wav_path}")
                    except Exception as e_wave_backend:
                        print(f"[WAVEFORM-ERROR] wave backend failed for {wav_path}: {e_wave_backend}")
                        print(f"[THUMBNAIL-DEBUG] Returning None at wav backend failure")
                        return None
                else:
                    # Non-wav fallback: try pydub for MP3s (since soundfile failed/absent)
                    if wav_path.lower().endswith('.mp3'):
                        try:
                            ...
                            if audio.channels > 1:
                                data = samples[::audio.channels]
                            else:
                                data = samples
                            data = list(data)
                            print(f"[THUMBNAIL-DEBUG] Finished pydub block for: {wav_path}")
                        except Exception as e_pydub:
                            import traceback
                            print(f"[WAVEFORM-ERROR] pydub fallback failed for {wav_path}: {e_pydub}\n{traceback.format_exc()}")
                            print(f"[THUMBNAIL-DEBUG] Returning None at pydub fallback failure")
                            return None
                    else:
                        print(f"[WAVEFORM-ERROR] No available backend succeeded for {wav_path}")
                        print(f"[THUMBNAIL-DEBUG] Returning None at no backend available")
                        return None
        except Exception as e_wave:
            import traceback
            print(f"[WAVEFORM-ERROR] Exception in waveform extraction for {wav_path}: {e_wave}\n{traceback.format_exc()}")
            print(f"[THUMBNAIL-DEBUG] Returning None at exception in waveform extraction")
            return None
        # Downsample to width
        ...
        painter.end()
        # Save thumbnail to disk for persistent cache (as .thumb, PNG format)
        saved = pixmap.save(thumb_path, 'PNG')
        if saved:
            print(f"[THUMBNAIL-CACHE] Saved new thumbnail: {thumb_path}")
        else:
            print(f"[THUMBNAIL-CACHE] Failed to save thumbnail: {thumb_path}")
        return pixmap
    except Exception as e:
        import traceback
        print(f"[WAVEFORM-ERROR] Exception for {wav_path}: {e}\n{traceback.format_exc()}")
        return None
    ...
    def refresh(self):
        ...
        def on_finished(entries, error):
            ...
            if error:
                ...
                audio_exts = ['.wav', '.flac', '.ogg', '.aiff', '.aif', '.aifc', '.au', '.snd', '.sf', '.caf', '.mp3']
                        ...
                        try:
                            ...
                            class SimpleCache:
                                ...
                                def put(self, key, value):
                                    cache_put(key, value)
                            print(f"[THUMBNAIL-DEBUG] Calling get_waveform_thumbnail for: {audio_path}")
                            ...
                            if ext in supported_audio_exts:
                                ...
                                print(f"[THUMBNAIL-DEBUG] Skipping unsupported audio extension for thumbnail: {audio_path}")
...
def precache_text_pdf_thumbnails_in_directory(directory, thumbnail_cache, size=128, max_workers=4, on_complete=None, parent=None, show_progress=False):
    ...
    print(f"[THUMBNAIL-PRECACHE] Files needing caching: {len(files_to_cache)}")
    ...
    def cache_one_file(file_path):
        ...
        try:
            ...
            if ext in text_exts:
                ...
                print(f"[THUMBNAIL-PRECACHE] Generating waveform thumbnail for {file_path}")
                try:
                    print(f"[THUMBNAIL-DEBUG] Calling get_waveform_thumbnail for: {file_path}")
                    ...
                    print(f"[THUMBNAIL-PRECACHE] About to cache waveform thumbnail for {file_path}, {len(png_bytes)} bytes")
                    thumbnail_cache.put(file_path, size, png_bytes)
                    print(f"[THUMBNAIL-PRECACHE] Cached waveform thumbnail for {file_path}")
                except Exception as e:
                    print(f"[THUMBNAIL-ERROR] Failed to generate waveform for {file_path}: {e}")
            elif ext in image_exts:
                print(f"[THUMBNAIL-PRECACHE] Generating image thumbnail for {file_path}")
                ...
                img.save(buf, format='PNG')
                png_bytes = buf.getvalue()
                print(f"[THUMBNAIL-PRECACHE] About to cache image thumbnail for {file_path}, {len(png_bytes)} bytes")
                thumbnail_cache.put(file_path, size, png_bytes)
                print(f"[THUMBNAIL-PRECACHE] Cached image thumbnail for {file_path}")
        except Exception as e:
            print(f"[THUMBNAIL-ERROR] Failed for {file_path}: {e}")
    print(f"[THUMBNAIL-PRECACHE] Starting cache thread pool for {len(files)} files (will process {len(files_to_cache)})")
    import concurrent.futures
    ...
    try:
        ...
        if getattr(e, 'errno', None) == errno.EXDEV:
            try:
                ...
                print(f"[MOVE-ERROR] fallback shutil.move failed: {e2}")
                raise
        else:
            # Other OSErrors: try shutil.move as a best-effort fallback
            try:
                shutil.move(src, final_dest)
                print(f"[MOVE-DEBUG] fallback shutil.move (other): {src} -> {final_dest}")
                return final_dest
            except Exception as e2:
...
def precache_video_thumbnails_in_directory(directory, thumbnail_cache, size=128, max_workers=4, parent=None, show_progress=False):
    ...
    print(f"[THUMBNAIL-PRECACHE-VIDEO] Found {len(video_files)} video files, to cache: {len(files_to_cache)}")
    # Consult user preference for caching dialog: 'always_show', 'ask', 'always_hide'
    try:
        settings = QSettings("garysfm", "garysfm")
        cache_dialog_pref = settings.value('cache_dialog_pref', 'ask')
    ...
    def cache_one_video(video_path):
        if thumbnail_cache.get(video_path, size) is not None:
            print(f'[THUMBNAIL] Already cached: {video_path}')
            return
        try:
            import ffmpeg
            from PIL import Image
            import tempfile
            print(f'[THUMBNAIL] Probing video: {video_path}')
            # Use ffmpeg-python with custom ffmpeg path if needed
            probe = ffmpeg.probe(video_path, cmd=ffmpeg_path)
            duration = float(probe['format'].get('duration', 0))
            if duration == 0:
                print(f'[THUMBNAIL-ERROR] Could not determine duration for {video_path}')
                return
            seek_time = max(duration * 0.1, 1.0)
            with tempfile.NamedTemporaryFile(suffix='.jpg', delete=False) as tmp:
                tmp_path = tmp.name
            print(f'[THUMBNAIL] Extracting frame at {seek_time:.2f}s to {tmp_path}')
            (
                ffmpeg
            ...
            if not os.path.exists(tmp_path) or os.path.getsize(tmp_path) == 0:
                print(f'[THUMBNAIL-ERROR] ffmpeg did not produce a valid thumbnail for {video_path}')
                return
            try:
                ...
                print(f'[THUMBNAIL-ERROR] PIL failed to process thumbnail for {video_path}: {e}')
                os.remove(tmp_path)
                return
            qimg = QPixmap(tmp_path)
            if qimg.isNull():
                print(f'[THUMBNAIL-ERROR] QPixmap failed to load thumbnail for {video_path} (trying QImage fallback)')
                from PyQt5.QtGui import QImage
                try:
                    img_fallback = QImage(tmp_path)
                    if not img_fallback.isNull():
                        qimg = QPixmap.fromImage(img_fallback)
                        print(f'[THUMBNAIL-DEBUG] QImage fallback succeeded for {video_path}')
                    else:
                        print(f'[THUMBNAIL-ERROR] QImage fallback also failed for {video_path}')
                        os.remove(tmp_path)
                        return
                except Exception as e:
                    print(f'[THUMBNAIL-ERROR] Exception in QImage fallback for {video_path}: {e}')
                    os.remove(tmp_path)
                    return
            os.remove(tmp_path)
            thumbnail_cache.put(video_path, size, qimg)
            print(f'[THUMBNAIL] Cached video thumbnail for {video_path}')
        except Exception as e:
            import traceback
            print(f'[THUMBNAIL-ERROR] Exception for {video_path}: {e}')
            traceback.print_exc()
    executor = concurrent.futures.ThreadPoolExecutor(max_workers=max_workers)
    from concurrent.futures import as_completed
...
Version: 1.1.4 - Beginning ISO support, My Computer (drive list), and other improvements

A cross-platform file manager built with PyQt5, supporting Windows, macOS, and Linux.



WHAT'S NEW IN 1.1.3 (August 2025):
- New: Tools → Recursive Precache Thumbnails... for fast, multi-size thumbnail generation across folders
- New: Always shows a progress dialog during thumbnail caching (no more preferences dialog)
- New: Thread-safe thumbnail generation (no more Qt thread warnings)
- Improved: No extra popups during recursive precache; only the progress dialog is shown
...
class WordWrapDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        """Custom paint method that handles truncation, word wrapping, and icon drawing"""
        ...
        painter.save()
        text_rect = QRect(rect.left() + text_offset, rect.top() + 2, rect.width() - text_offset - 3, rect.height() - 4)
...
class DirectorySelectionDialog(QDialog):
    ...
    def navigate_to(self, path):
        ...
        if os.path.isdir(path):
            ...
            self.selected_directory = path
            self.selected_label.setText(path)
                # Do NOT update the left tree view here — keep the left tree showing drives
                # (previous behavior updated the left tree to follow tab navigation; we disable that)
        if os.path.isdir(path):
            self.navigate_to(path)
        else:
            self.address_bar.setStyleSheet("background-color: #ffcccc;")
            # Optionally, show a tooltip or error
...
class IconWidget(QWidget):
    ...
    def __init__(self, file_name, full_path, is_dir, thumbnail_size=64, thumbnail_cache=None, use_icon_only=False, parent=None):
        ...
        # When True, draw simple icons only (no image/video/audio thumbnails)
        ...
        # Make label width match thumbnail width so base names can be as wide as thumbnails
        try:
            self.label.setFixedWidth(self.thumbnail_size)
        except Exception:
            pass
        # Set initial text after sizing
    ...
    # Determine effective icon-only mode: respect the widget flag or fall back to main window's view mode
        ...
        try:
            ...
            pass
        # Debug
        print(f'[THUMBNAIL-DEBUG] effective_icon_only={effective_icon_only} for {full_path}')
        ...
        try:
            # ...existing code for file icon/thumbnail drawing...
                ...
                if self.thumbnail_cache and (file_ext in text_exts or file_ext in pdf_exts or file_ext in docx_exts or file_ext in audio_exts):
                    print(f'[THUMBNAIL-DEBUG] Entered text/pdf/docx/audio cache/painter block for {full_path}')
                    ...
                    pixmap = None
                    if cached_thumb:
                        print(f'[THUMBNAIL-DEBUG] Using cached text/pdf/docx/audio thumbnail for {full_path}')
                    ...
                    if not pixmap and file_ext in supported_audio_exts:
                        print(f'[THUMBNAIL-DEBUG] No cached waveform for {full_path}, generating on the fly')
                        try:
                            print(f"[THUMBNAIL-DEBUG] Calling get_waveform_thumbnail for: {full_path}")
                            ...
                            if not pixmap.isNull():
                                ...
                                print(f'[THUMBNAIL-DEBUG] Cached waveform thumbnail for {full_path} after on-the-fly gen')
                        except Exception as e:
                            print(f'[THUMBNAIL-ERROR] Failed to generate waveform for {full_path} in icon view: {e}')
                    if pixmap and not pixmap.isNull() and pixmap.width() > 0 and pixmap.height() > 0:
                        painter.drawPixmap(0, 0, pixmap)
                        painter.end()
                        print(f'[THUMBNAIL-DEBUG] Returning framed_pixmap for {full_path} (valid cached pixmap or on-the-fly)')
                        return framed_pixmap
                    else:
                        print(f'[THUMBNAIL-DEBUG] Cached pixmap for {full_path} is invalid or not found, drawing default icon')
                        self.draw_default_file_icon(painter, full_path, size)
                        painter.end()
                ...
                if ArchiveManager.is_archive(full_path):
                    ...
                    if sys.platform == 'darwin':
                        ...
                        if cached_thumb:
                            print(f'[THUMBNAIL-DEBUG] Using cached video thumbnail for {full_path}')
                            if isinstance(cached_thumb, bytes):
                                pixmap = QPixmap()
                                pixmap.loadFromData(cached_thumb, 'PNG')
                                print(f'[THUMBNAIL-DEBUG] Loaded pixmap from bytes for {full_path}, isNull={{pixmap.isNull()}}')
                            ...
                            if not pixmap.isNull() and pixmap.width() > 0 and pixmap.height() > 0:
                                painter.drawPixmap(0, 0, pixmap)
                                painter.end()
                                print(f'[THUMBNAIL-DEBUG] Returning framed_pixmap for {full_path} (valid cached video pixmap)')
                                return framed_pixmap
                            else:
                                print(f'[THUMBNAIL-DEBUG] Cached video pixmap for {full_path} is invalid, drawing default icon')
                                self.draw_default_file_icon(painter, full_path, size)
                                painter.end()
                                print(f'[THUMBNAIL-DEBUG] Returning framed_pixmap for {full_path} (default icon)')
                                return framed_pixmap
                        # If not cached, generate on demand (like other platforms)
                        print(f'[THUMBNAIL-DEBUG] No cached video thumbnail for {full_path}, generating on demand')
                        try:
                            import shutil
                            ffmpeg_path = shutil.which('ffmpeg')
                            if not ffmpeg_path:
                                ...
                                try:
                                    ...
                                    if video_pixmap.isNull():
                                        ...
                                        if not img_fallback.isNull():
                                            ...
                                            print(f'[THUMBNAIL-DEBUG] QImage fallback succeeded for {full_path}')
                                        else:
                                            print(f'[THUMBNAIL-DEBUG] QImage fallback also failed for {full_path}')
                            ...
                            print(f'[THUMBNAIL-DEBUG] Exception in macOS video thumbnail code for {full_path}: {e}')
                            self.draw_default_file_icon(painter, full_path, size)
                        painter.end()
                        return framed_pixmap
    ...
    def draw_custom_archive_icon(self, painter, archive_path, size):
        ...
        if archive_type == '.zip':
            ...
            try:
                ...
                if image_entry:
                    ...
                    if cache and cache.is_cached(cache_key_path, size):
                        ...
                        try:
                            ...
                            img_pix = None
                    # If we obtained an image pixmap, draw disc then overlay
                    if 'img_pix' in locals() and img_pix and not img_pix.isNull():
                        # Draw disc beneath as fallback background
                        from PyQt5.QtGui import QRadialGradient
                        center = QPoint(size // 2, size // 2)
                        radius = size // 2 - 4
                        grad = QRadialGradient(center, radius)
                ...
                # Fall back to disc-style icon below
                pass
    ...
    def mousePressEvent(self, event):
        try:
            print(f"[ICON-CONTAINER] mousePressEvent button={getattr(event, 'button', lambda: None)()} pos={getattr(event, 'pos', lambda: '<no-pos>')()} global={getattr(event, 'globalPos', lambda: '<no-global>')()} modifiers={getattr(event, 'modifiers', lambda: 0)()}")
        except Exception:
            pass
...
class IconContainer(QWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if is_empty_space:
                try:
                    print(f"[ICON-CONTAINER] Right click in container empty space at pos={event.pos()} global={event.globalPos()}")
                except Exception:
                    pass
    ...
    def mouseReleaseEvent(self, event):
        try:
            print(f"[ICON-CONTAINER] mouseReleaseEvent button={getattr(event, 'button', lambda: None)()} pos={getattr(event, 'pos', lambda: '<no-pos>')()} global={getattr(event, 'globalPos', lambda: '<no-global>')()}")
        except Exception:
            pass
    ...
    def refresh_thumbnail_view(self):
        ...
        if getattr(self, 'is_drive_list', False):
            try:
                ...
                # Enumerate drives using QDir.drives()
                ...
                for d in drives:
                    try:
                        ...
                        # Wrap double-click to log and then call handler
                        def _drv_dd(p, tab=self):
                            try:
                                # placeholder - drive icon double-clicked
                                _ = None
                            except Exception:
                                pass
                            try:
                                # If this is a My Computer tab, directly navigate into the drive
...
class SimpleFileManager(QMainWindow):
    ...
    def setup_enhanced_keyboard_shortcuts(self):
        ...
        # View modes
        QShortcut(QKeySequence(f"{main_modifier}+1"), self, lambda: self.set_view_mode(ViewModeManager.THUMBNAIL_VIEW))
    ...
    def eventFilter(self, obj, event):
        ...
        if (hasattr(current_tab, 'scroll_area') and hasattr(current_tab.scroll_area, 'viewport') and
            ...
            try:
                ...
                evt_name = '<unknown>'
            # Only print a small amount to avoid flooding logs
            print(f"[EVENT-FILTER] Received event type={event.type()} obj={repr(obj)}")
    ...
    def show_about_dialog(self):
        ...
        about_lines = [
            ...
            "Release: August 2025",
            "",
            "Announcement: This release introduces early ISO image browsing support and a new 'My Computer' view to list connected drives.",
            "",
            "🌟 WHAT'S NEW IN 1.1.4:",
            "• Beginning ISO image browsing support (mountless listing and basic navigation)",
            "• New 'My Computer' drive-list view: lists connected drives in the tree and as a tab",
            "• Double-click a drive in My Computer to enter it; Up from a drive root returns to My Computer",
            "• Back/Forward navigation now supports My Computer ↔ directory traversal",
            "• My Computer tabs follow the global view-mode (Thumbnail/List/Detail)",
            "",
            "🚀 FEATURES (including previous versions and 1.1.3 updates):",
            "• Tools → Recursive Precache Thumbnails... for multi-size, fast thumbnail generation (introduced in 1.1.3)",
            "• Always shows a progress dialog during thumbnail caching (1.1.3)",
            "• Thread-safe thumbnail generation (1.1.3)",
            "• No extra popups during recursive precache; only the progress dialog is shown (1.1.3)",
            "• General stability, performance, and reliability improvements (1.1.3)",
            "• Video thumbnailing for major formats (mp4, mkv, avi, mov, etc.)",
            "• ffmpeg-based thumbnail extraction (cross-platform)",
            "• Persistent thumbnail cache for images and videos",
            ...
            "• Beginning ISO image browsing support (mountless listing and basic navigation)",
            "• 'My Computer' drive-list view showing connected drives",
