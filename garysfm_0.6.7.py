def precache_video_thumbnails_in_directory(directory, thumbnail_cache, size=128, max_workers=4):
    ...
    def cache_one_video(video_path):
        ...
        try:
            # Use ffmpeg to extract thumbnail (same logic as in create_icon_or_thumbnail)
            import ffmpeg
            ...
            # Convert to QPixmap for cache compatibility
...
Version: 0.6.7 - Video Thumbnails, ffmpeg Support & Stability Improvements
    ...
    def navigate_to_path(self, path):
        ...
        try:
            if os.path.exists(path) and os.path.isdir(path):
                ...
                # Automatically pre-cache video thumbnails in the background
                if hasattr(self, 'thumbnail_cache') and self.thumbnail_cache:
                    try:
                        precache_video_thumbnails_in_directory(path, self.thumbnail_cache, size=getattr(self, 'thumbnail_size', 128))
                    except Exception as e:
                        print(f"[DEBUG] Error starting video thumbnail pre-cache: {e}")
            else:
